...
if (_isWindows() && mongo_hacker_config['windows_warning']) {
  ...
  print(
    'the default Windows Command Prompt. If you are using an alternative console '
  )
  print(
    'such as ConEmu (https://github.com/Maximus5/ConEmu) you may wish to try enabling'
  )
...
if (typeof db !== 'undefined') {
  ...
  if (shell_version < 2.4 || server_version < 2.4) {
    ...
    print(
      'Sorry! MongoDB Shell Enhancements for Hackers is only compatible with MongoDB 2.4+'
    )
    print(
      '~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~'
    )
    ...
    print(
      'WARNING: Your shell version (' +
        shell_version +
        ') ' +
        'or server version (' +
        server_version +
        ') of MongoDB is end of life\n'
    )
    print(
      'You should consider upgrading to a supported version (currently 3.4 or newer):'
    )
...
DBCollection.prototype.aggregate = function (pipeline, aggregateOptions) {
  if (pipeline instanceof Array) {
    ...
    print(
      'WARNING: Legacy aggregate() syntax does not support passing options.'
    )
...
DB.prototype.rename = function (newName) {
  if (newName == this.getName() || newName.length === 0) return
...
DB.prototype.indexStats = function (collectionFilter, details) {
  ...
  for (i = 0; i < collectionNames.length; i++) {
    print(
      colorize(
        collectionNames[i].pad(maxKeyLength, true),
    ...
    var totalIndexSize =
      Math.round((stats.totalIndexSize / 1024 / 1024) * 10) / 10 + ' MB'

    var indexNames = []
    var indexSizes = []
    for (indexName in stats.indexSizes) {
      indexSizes.push(
        Math.round((stats.indexSizes[indexName] / 1024 / 1024) * 10) / 10 +
          ' MB'
      )
    ...
    print(
      colorize(
        'totalIndexSize'.pad(maxKeyLength, true),
        ...
        ' ' +
        colorize(
          totalIndexSize.pad(maxValueLength),
          mongo_hacker_config['colors']['number']
        )
    )
    ...
    if (details) {
      for (var j = 0; j < indexSizes.length; j++) {
        print(
          colorize(
            '' + indexNames[j].pad(maxIndexKeyLength, true),
            mongo_hacker_config['colors']['string']
          ) +
            ' ' +
            columnSeparator +
            ' ' +
            colorize(
              indexSizes[j].pad(maxIndexValueLength),
              mongo_hacker_config['colors']['binData']
            )
        )
      }
    }
...
DBQuery.prototype.upsert = function (upsert) {
  ...
  var startTime =
    typeof _verboseShell === 'undefined' || !_verboseShell
      ? 0
      : new Date().getTime()
...
function listDbs() {
  return db.adminCommand('listDatabases').databases.map(function (d) {
    return d.name
  })
}
  ...
  if (query !== '') {
    ...
    for (var command in result.commands) {
      ...
      if (regexp.test(command) || regexp.test(help)) {
        ...
        print(
          colorize(command, { color: 'green' }),
          Array(numSpaces).join(' '),
          '-',
          help
        )
      }
...
function printPaddedColumns() {
  var columnWidths = Array.prototype.map.call(arguments, function (column) {
...
DBCollection.prototype.agg_group = function (
  name,
  group_field,
  operation,
  op_value,
...
var psHack = {
  ...
  ps: function (db, argumentString) {
    ...
    if (options['h'] != undefined || options['help'] != undefined) {
      ...
      print('Options:')
      print(
        '  -a | --all                       Show all operations (this overrides all other options)'
      )
      print('  -c | --client <ip/hostname>      Show process for client*')
      print(
        '  -d | --description <description> Show processes with description'
      )
      print('  -A | --active                    Show only active processes')
      print(
        '  -r | --running <seconds>         Show only processes which have been running for a certain time'
      )
      print(
        '  -o | --op <operation>            Show commands running a certain operation'
      )
      print(
        '  -n | --ns <namespace>            Show commands running against a certain namespace*'
      )
      print(
        '  -x | --app <app name>            Show commands with a matching app name*'
      )
      print(
        '  -l | --locks                     Show commands waiting for a lock'
      )
      print(
        '  -C | --collection <name>         Search the given collection instead of calling db.currentOp()'
      )
      print(
        '  -v | --verbose                   Print the query that is being run'
      )
      print(
        'Note: Options marked with * will return all operations that start with the provided value.'
      )
      return
    }
    ...
    var clientQuery = this.getOptionsAsQuery(
      options,
      ['c', 'client'],
      function (x) {
        return { $regex: '^' + x }
...
function randomDate(start, end) {
  ...
  return new Date(
    start.getTime() + Random.randInt(end.getTime() - start.getTime())
  )
}
// Better show dbs
...
function uuidToString(uuid, uuidType) {
  ...
  var hex = platformSpecificUuidModifications[uuidType](
...
DBCollection.prototype._printExtraInfo = function (action, startTime) {
  ...
  if (res) {
    if (res.err != undefined && res.err != null) {
      if (
        res.errmsg &&
        res.errmsg !== 'This command is not supported in transactions'
      ) {
        print(res.err)
      }
      return
